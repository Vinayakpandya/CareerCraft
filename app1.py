# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 13:17:08 2024
...
def create_usertable():
    c.execute('CREATE TABLE IF NOT EXISTS usertable(User TEXT,Firstname TEXT,Lastname TEXT,Mobile TEXT,Email TEXT,City TEXT,Password TEXT,Cpassword TEXT)')
def add_userdata(User,FirstName,LastName,Mobile,City,Email,password,Cpassword):
    c.execute('INSERT INTO usertable(User,FirstName,LastName,Mobile,City,Email,password,Cpassword) VALUES (?,?,?,?,?,?,?,?)',(User,FirstName,LastName,Mobile,City,Email,password,Cpassword))
    conn.commit()
def login_user(User,Email,password):
    c.execute('SELECT * FROM usertable WHERE User=? AND Email =? AND password =?', (User,Email,password))
    data = c.fetchall()
    return data  
def view_all_users():
...
def delete_user(Email):
    ...
    conn.commit()
def create_job():
    c.execute('CREATE TABLE IF NOT EXISTS usertable1(Email TEXT,JT TEXT,JR TEXT,JP TEXT,JL TEXT)')
def add_job(Email,JT,JR,JP,JL):
    c.execute('INSERT INTO usertable1(Email,JT,JR,JP,JL) VALUES (?,?,?,?,?)',(Email,JT,JR,JP,JL)) 
    conn.commit()
def view_all_job(Email):
    c.execute("SELECT * FROM usertable1 WHERE Email=?", (Email,))
    data = c.fetchall()
    return data
def delete_job(JT):
    c.execute("DELETE from usertable1 WHERE JT="+"'"+JT+"'")
    conn.commit()
def update_pass(Email,Password):
    c=conn.cursor()
    c.execute('UPDATE usertable SET password="'+Password+'",Cpassword="'+Password+'" WHERE Email=="'+Email+'";')
    conn.commit()
def search_job(sj):
    c.execute("SELECT * from usertable1 WHERE JR=?", (sj,))
    data = c.fetchall()
    return data
def check_prof():
    c.execute("SELECT * FROM usertable WHERE user=?", ("User",))
    data = c.fetchall()
    return data
            ...
            if result:
                ...
                if Usr=="User":
                    ...
                    if choice3=="Build Resume":
                        ...
                        choiceT = st.selectbox("No of TechnicalSkill",menuN)
                        TechSkillDesc=[]
            ...
            if Password==CPassword:
                if (pattern.match(Mname)):
                    if re.fullmatch(regex, Email):
                        ...
                        st.info("Go to Login Section for Login")
                    else:
                        st.warning("Not Valid Email")         
                else:
